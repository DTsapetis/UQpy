...
from UQpy.run_model import RunModel, ThirdPartyModel
...
def test_model_script():
    with pytest.raises(ValueError):
        model = PythonModel(
            model_script="random_file_name", model_object_name="SumRVs", delete_files=True
        )
        runmodel_object = RunModel(model=model)
...
def test_samples():
    with pytest.raises(BeartypeCallHintParamViolation):
        model = PythonModel(
            model_script="python_model.py", model_object_name="SumRVs", delete_files=True
        )
...
def test_python_serial_workflow_class_vectorized():
    ...
    assert np.allclose(
        np.array(model_python_serial_class.qoi_list).flatten(), np.sum(x_mcs.samples, axis=1)
    )
...
def test_append_samples_false():
    ...
    assert np.allclose(
        np.array(model_python_serial_class.qoi_list).flatten(), np.sum(x_mcs_new.samples, axis=1)
    )
...
def test_python_serial_workflow_function_vectorized():
    ...
    assert np.allclose(
        np.array(model_python_serial_function.qoi_list).flatten(), np.sum(x_mcs.samples, axis=1)
    )
...
def test_python_parallel_workflow_class():
    ...
    assert np.allclose(
        np.array(model_python_parallel_class.qoi_list).flatten(), np.sum(x_mcs.samples, axis=1)
    )
...
def test_python_parallel_workflow_function():
    ...
    assert np.allclose(
        np.array(model_python_parallel_function.qoi_list).flatten(), np.sum(x_mcs.samples, axis=1)
    )
