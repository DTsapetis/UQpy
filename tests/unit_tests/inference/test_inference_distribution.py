...
prior2 = JointIndependent([Uniform(loc=0.0, scale=0.5), Lognormal(s=1.0, loc=0.0, scale=1.0)])
...
def test_bayes_mcmc():
    ...
    bayes_estimator = BayesParameterEstimation(
        sampling_class=mh1, inference_model=candidate_model_no_prior, data=data, nsamples=50
    )
...
def test_bayes_selection():
    ...
    selection = BayesModelSelection(
        parameter_estimators=[parameter_estimator, parameter_estimator1], nsamples=[50, 50]
    )
