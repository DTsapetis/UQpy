from UQpy.distributions.collection import ChiSquare, Exponential, Gamma
...
def test_aic():
    ...
    selector = InformationModelSelection(
        parameter_estimators=[mle1, mle2, mle3], criterion=AIC(), n_optimizations=[5] * 3
    )
...
def test_bic():
    ...
    mle3 = MLE(inference_model=m2, random_state=0, data=data)
    selector = InformationModelSelection(
        parameter_estimators=[mle1, mle2, mle3], criterion=BIC(), n_optimizations=[5] * 3
    )
    selector.sort_models()
...
def test_aicc():
    ...
    mle3 = MLE(inference_model=m2, random_state=0, data=data)
    selector = InformationModelSelection(
        parameter_estimators=[mle1, mle2, mle3], criterion=AICc(), n_optimizations=[5] * 3
    )
