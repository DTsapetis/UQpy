...
def test_delaunay_sts():
    ...
    sts_obj = TrueStratifiedSampling(
        distributions=marginals, strata_object=strata_obj, random_state=1, nsamples_per_stratum=1
    )
...
def test_voronoi_sts():
    ...
    x = TrueStratifiedSampling(
        distributions=marginals, strata_object=strata, nsamples_per_stratum=3
    )
...
strata1 = RectangularStrata(
    strata_number=[3, 3], sampling_criterion=SamplingCriterion.CENTERED, random_state=1
)
x_sts1 = TrueStratifiedSampling(
    distributions=marginals, strata_object=strata1, nsamples_per_stratum=1
)
...
sts_del = TrueStratifiedSampling(
    distributions=marginals, strata_object=strata_del, nsamples_per_stratum=2
)
...
def test_rect_random():
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [1.14966929, 1.27457918],
                [1.4055223, 1.1062248],
                [2.25732188, 1.03126317],
                [1.0640978, 1.59515015],
                [2.60406483, 1.71936573],
                [1.15051073, 3.254492],
                [1.51330216, 4.20330958],
                [2.12638191, 3.20869262],
            ]
        ),
    )
    assert np.allclose(
        ...
        np.array(
            [
                [0.13900733, 0.24010816],
                [0.33337146, 0.10077752],
                [0.7155853, 0.03077953],
                [0.06208674, 0.44852024],
                [0.79892249, 0.51293891],
                [0.1397315, 0.89507317],
                [0.40148408, 0.95937248],
                [0.67579586, 0.89015584],
            ]
        ),
    )
...
def test_rect_nsamples_check1():
    ...
    with pytest.raises(ValueError):
        ...
        TrueStratifiedSampling(
            distributions=marginals, strata_object=strata, nsamples_per_stratum=1, nsamples=10
        )
...
def test_vor_nsamples_per_strarum_not_0():
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [1.5185188, 2.50692166],
                [1.19961165, 2.53070353],
                [4.64208735, 3.15996289],
                [1.1938551, 1.30464598],
                [1.22346669, 1.46101267],
                [1.00251352, 1.68941482],
                [1.58543262, 1.07725168],
                [2.12618711, 1.66531153],
            ]
        ),
    )
    assert np.allclose(
...
def test_vor_random_state():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        TrueStratifiedSampling(
            distributions=marginals, strata_object=strata_vor, random_state="abc"
        )
...
def test_del_nsamples_per_strarum_not_0():
    ...
    assert np.allclose(
        ...
        np.array(
            [
                [1.14604405, 3.46440724],
                [1.10919352, 1.98225373],
                [1.74510444, 1.4267083],
                [1.95844896, 1.14825374],
                [1.87318612, 3.40220043],
                [1.60852035, 4.16117521],
                [3.23430303, 2.49568182],
                [3.89471882, 2.03363426],
            ]
        ),
    )
