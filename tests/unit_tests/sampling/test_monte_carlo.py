...
from UQpy.distributions import MultivariateNormal, Normal
...
z1.run(nsamples=2)
z2 = MonteCarloSampling(
    distributions=[dist1, dist2], nsamples=2, random_state=np.random.RandomState(123)
)
z2.run(nsamples=2)
...
z4 = MonteCarloSampling(
    distributions=[MultivariateNormal([0, 0])], nsamples=2, random_state=np.random.RandomState(123)
)
z4.run(nsamples=2)
z4.transform_u01()
...
def test_samples1():
    ...
    expected_samples = np.array([[-1.09], [1.0], [0.28], [-1.51], [-0.58]])
...
def test_samples2():
    ...
    expected_samples = np.array(
        [[-1.09, 1.65], [1.0, -2.43], [0.28, -0.43], [-1.51, 1.27], [-0.58, -0.87]]
    )
...
def test_samples3():
    ...
    expected_samples = np.array([[-1.09], [1.0], [0.28], [-1.51]])
...
def test_samples4():
    ...
    expected_samples = np.array([[-1.09, 0.28], [1.0, -1.51], [-0.58, -2.43], [1.65, -0.43]])
...
def test_samples5():
    ...
    expected_samples = np.array(
        [[[-1.09, 1.0]], [[0.28, -1.51]], [[-0.58, 1.65]], [[-2.43, -0.43]]]
    )
...
def test_samples2_u01():
    ...
    expected_samples_u01 = np.array(
        [[0.14, 0.95], [0.84, 0.01], [0.61, 0.33], [0.07, 0.9], [0.28, 0.19]]
    )
...
def test_nsamples_none():
    ...
    with pytest.raises(BeartypeCallHintParamViolation):
        MonteCarloSampling(distributions=dist1, random_state=np.random.RandomState(123)).run(
            nsamples=None
        )
