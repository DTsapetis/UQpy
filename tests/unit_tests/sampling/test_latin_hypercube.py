...
def test_lhs_random_criterion():
    ...
    latin_hypercube_sampling = LatinHypercubeSampling(
        distributions=distribution, nsamples=2, criterion=random_criterion, random_state=1
    )
...
def test_lhs_centered_criterion():
    ...
    latin_hypercube_sampling = LatinHypercubeSampling(
        distributions=distribution, nsamples=2, criterion=centered_criterion, random_state=1
    )
    actual_samples = latin_hypercube_sampling._samples.flatten()
...
def test_lhs_maximin_criterion():
    ...
    latin_hypercube_sampling = LatinHypercubeSampling(
        distributions=distribution, nsamples=2, criterion=maximin_criterion, random_state=1
    )
    actual_samples = latin_hypercube_sampling._samples.flatten()
    expected_samples = np.array([[1.208511], [1.86016225]]).flatten()
    np.testing.assert_allclose(expected_samples, actual_samples, rtol=1e-6)
...
def test_lhs_maximin_criterion_joint_independent():
    maximin_criterion = MaxiMin()
    latin_hypercube_sampling = LatinHypercubeSampling(
        distributions=JointIndependent(marginals=[distribution, distribution1]),
        nsamples=2,
...
x1b = LatinHypercubeSampling(
    distributions=joint_dist, criterion=MinCorrelation(), nsamples=5, random_state=123
)

x1d = LatinHypercubeSampling(
    distributions=dist1, criterion=Centered(), random_state=123, nsamples=5
)
...
x1h = LatinHypercubeSampling(
    distributions=[dist1, dist2], criterion=MaxiMin(), nsamples=5, random_state=789
)
...
def test_samples5():
    ...
    expected_samples = np.array([[0.56, 0.2], [0.25, 0.62], [1.0, 0.4], [0.72, 0.91], [0.06, 0.15]])
