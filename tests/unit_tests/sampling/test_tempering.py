...
from UQpy.distributions import JointIndependent, MultivariateNormal, Uniform
...
def log_prior(x):
    loc, scale = -20.0, 40.0
    return Uniform(loc=loc, scale=scale).log_pdf(x[:, 0]) + Uniform(loc=loc, scale=scale).log_pdf(
        x[:, 1]
    )
...
seed = -2.0 + 4.0 * random_state.rand(5, 2)
betas = [1.0 / np.sqrt(2.0) ** i for i in range(10 - 1, -1, -1)]
...
def test_parallel():
    samplers = [
        MetropolisHastings(burn_length=10, jump=2, seed=list(seed), dimension=2)
...
def test_thermodynamic_integration():
    ...
    mcmc.run(nsamples_per_chain=100)
    log_ev = mcmc.evaluate_normalization_constant(compute_potential=compute_potential, log_Z0=0.0)
...
def test_sequential_seed_given():
    ...
    samps = np.array(
        [
            uniform.rvs(loc=-2.0, scale=4.0, size=100, random_state=960242069),
            uniform.rvs(loc=-2.0, scale=4.0, size=100, random_state=24969420),
        ]
    ).T
    test = SequentialTemperingMCMC(
        pdf_intermediate=likelihood,
