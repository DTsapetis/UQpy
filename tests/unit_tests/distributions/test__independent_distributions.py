...
def test_gen_extreme():
    assert GeneralizedExtreme(c=2.0).cdf(x=0.8) == 1.0


def test_inverse_gauss():
    assert np.round(InverseGauss(mu=2.0).cdf(x=0.8), 3) == 0.411
...
def test_truncated_normal():
    assert np.round(TruncatedNormal(a=-1.0, b=1.0).cdf(x=0.8), 3) == 0.922
...
def test_multinomial_4():
    ...
    assert np.all(moments == [1.0, 1.5, 2.5])
...
def test_mvnormal_1():
    assert np.round(MultivariateNormal(mean=[1.0, 2.0], cov=3.0).cdf(x=[0.8, 0.8]), 3) == 0.111


def test_mvnormal_2():
    assert np.round(MultivariateNormal(mean=[1.0, 2.0], cov=3.0).pdf(x=[0.8, 0.8]), 3) == 0.041


def test_mvnormal_3():
    assert np.round(MultivariateNormal(mean=[1.0, 2.0], cov=3.0).log_pdf(x=[0.8, 0.8]), 3) == -3.183
...
def test_mvnormal_5():
    samples = MultivariateNormal(mean=[1.0, 2.0], cov=1.0).rvs(nsamples=3, random_state=123)
    assert np.all(
        np.round(samples, 3) == np.array([[-0.086, 2.997], [1.283, 0.494], [0.421, 3.651]])
    )


def test_mvnormal_6():
    assert np.all(
        MultivariateNormal(mean=[1.0, 2.0], cov=3.0).moments(moments2return="m") == [1.0, 2.0]
    )


def test_mvnormal_7():
    assert np.all(MultivariateNormal(mean=[1.0, 2.0], cov=3.0).moments(moments2return="v") == 3.0)
...
def test_clayton():
    assert np.round(Clayton(theta=2.0).evaluate_cdf(unit_uniform_samples=unif), 3) == 0.393


def test_frank():
    assert np.round(Frank(theta=2.0).evaluate_cdf(unit_uniform_samples=unif), 3) == 0.379


def test_gumbel_1():
    assert np.round(Gumbel(theta=2.0).evaluate_cdf(unit_uniform_samples=unif), 3) == 0.398


def test_gumbel_2():
    assert np.round(Gumbel(theta=2.0).evaluate_pdf(unit_uniform_samples=unif), 3) == 0.261
...
def test_joint_ind_1():
    marginals_ = [Normal(loc=2.0, scale=2.0), Lognormal(s=1.0, loc=0.0, scale=np.exp(1))]
    dist_joint_ = JointIndependent(marginals=marginals_)
    dist_joint_.update_parameters(loc_0=3.0)
    assert dist_joint_.get_parameters()["loc_0"] == 3.0
...
def test_joint_ind_7():
    marginals_ = [Normal(loc=None, scale=2.0), Lognormal(s=1.0, loc=0.0, scale=np.exp(1))]
    dist_joint_ = JointIndependent(marginals=marginals_)
...
def test_joint_copula_1():
    ...
    assert dist_joint_.get_parameters()["theta_c"] == 2.0
