...
def test_2():
    ...
    Test tp basis
    """
    polynomial_basis = TensorProductBasis(distributions=dist, max_degree=max_degree).polynomials
...
def test_20():
    ...
    least_squares = LeastSquareRegression()
    pceGauss = PolynomialChaosExpansion(
        polynomial_basis=polynomial_basis, regression_method=least_squares
    )
    pceGauss.fit(x, x)
...
def test_21():
    ...
    assert all(
        (
            np.argwhere(np.round(pce2_lar_sens.calculate_generalized_total_order_indices(), 3) > 0)
            == [[0], [2], [3]]
        )
    )
...
def test_23():
    ...
    standardized_samples = polynomial_chaos.Polynomials.standardize_sample(
        X, pce.polynomial_basis.distributions
    )
    standardized_pdf = polynomial_chaos.Polynomials.standardize_pdf(
        X, pce.polynomial_basis.distributions
    )
...
def test_24():
    ...
    assert (
        tot_err < 10**-5
        and tot_err_ols < 10**-5
        and round(mean_res[50, 4], 3) == -1.5
        and round(mean_res[50, 3], 3) == 0
        and round(vartot_res[50, 3], 3) == 0
    )
