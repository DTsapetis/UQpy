...
from UQpy.distributions.collection import JointCopula, JointIndependent
...
class HyperbolicBasis(PolynomialBasis):
    def __init__(
        ...
        if 0 < hyperbolic < 1:
            mask = (
                np.round(np.sum(multi_index_set**hyperbolic, axis=1) ** (1 / hyperbolic), 4)
                <= max_degree
            )
        ...
        super().__init__(
            inputs_number, len(multi_index_set), multi_index_set, polynomials, distributions
        )
