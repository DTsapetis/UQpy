...
class MCMC(ABC):
        ...
        if self.evaluate_log_target is None and self.evaluate_log_target_marginals is None:
            (self.evaluate_log_target, self.evaluate_log_target_marginals) = (
                self._preprocess_target(
                    pdf_=self.pdf_target, log_pdf_=self.log_pdf_target, args=self.args_target
                )
            )
        # Initialize the runs: allocate space for the new samples and log pdf values
        (final_nsamples, final_nsamples_per_chain, current_state, current_log_pdf) = (
            self._initialize_samples(nsamples=nsamples, nsamples_per_chain=nsamples_per_chain)
        )
        ...
        if (
            ...
            if self.save_log_pdf:
                self.log_pdf_values = np.concatenate(
                    [self.log_pdf_values, np.zeros((nsamples_per_chain, self.n_chains))], axis=0
                )
    ...
    def _update_acceptance_rate(self, chain_state_acceptance=None):
        self.acceptance_rate = [
            na / self.iterations_number + (self.iterations_number - 1) / self.iterations_number * a
                ...
                evaluate_log_pdf_marginals = list(
                    map(
                        lambda i: lambda x: np.log(
                            np.maximum(pdf_[i](x, *args[i]), 10 ** (-320) * np.ones((x.shape[0],)))
                        ),
                        range(len(pdf_)),
                    )
                )
