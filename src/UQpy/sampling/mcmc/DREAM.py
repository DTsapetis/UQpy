...
class DREAM(MCMC):
    ...
    def __init__(
        ...
        for key, typ in zip(
            ["jump_rate", "c", "c_star", "crossover_probabilities_number", "gamma_probability"],
        ...
        for key in ["crossover_adaptation", "check_chains"]:
            p = getattr(self, key)
            if not (
                isinstance(p, tuple) and len(p) == 2 and all(isinstance(i, (int, float)) for i in p)
            ):
        ...
        self.cross_prob = (
            np.ones((self.crossover_probabilities_number,)) / self.crossover_probabilities_number
        )
    ...
    def run_one_iteration(self, current_state: np.ndarray, current_log_pdf: np.ndarray):
        ...
        unif_rvs = (
            Uniform()
            .rvs(nsamples=self.n_chains * (self.n_chains - 1), random_state=self.random_state)
        ...
        z = (
            Uniform()
            .rvs(nsamples=self.n_chains * self.dimension, random_state=self.random_state)
        ...
        for j in range(self.n_chains):
            for i in subset_a[j]:
                dx[j, i] = self.c_star * norm_vars[j, i] + (1 + lmda[j]) * g[j] * np.sum(
                    current_state[as_[j], i] - current_state[bs_[j], i]
                )
        ...
        unif_rvs = (
            Uniform().rvs(nsamples=self.n_chains, random_state=self.random_state).reshape((-1,))
        )
