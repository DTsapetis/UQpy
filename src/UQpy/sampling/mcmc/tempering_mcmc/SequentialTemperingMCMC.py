...
class SequentialTemperingMCMC(TemperingMCMC):
    ...
    def run(self, nsamples: PositiveInteger = None):
        ...
        while current_tempering_parameter < 1:
            ...
            current_tempering_parameter = self._find_temper_param(
                previous_tempering_parameter, points, self.evaluate_log_intermediate, nsamples
            )
            ...
            for i in range(self.n_resamples):
                ...
                if self.recalculate_weights:
                    weights[lead_index] = np.exp(
                        self.evaluate_log_intermediate(
                            points[lead_index, :].reshape((1, -1)), current_tempering_parameter
                        )
                        - self.evaluate_log_intermediate(
                            points[lead_index, :].reshape((1, -1)), previous_tempering_parameter
                        )
                    )
                    w_sum = np.sum(weights)
    ...
    def _find_temper_param(
        ...
        * **iter_thresh** ('float'):
        ...
        top = 1.0
        flag = (
            0  # Indicates when the tempering exponent has been found (flag = 1 => solution found)
        )
        loop_counter = 0
        while flag == 0:
            loop_counter += 1
