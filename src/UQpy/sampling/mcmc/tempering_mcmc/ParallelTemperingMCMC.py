...
class ParallelTemperingMCMC(TemperingMCMC):
    ...
    def __init__(
        ...
        if self.tempering_parameters is None:
            if self.n_tempering_parameters is None:
                ...
                isinstance(self.n_tempering_parameters, int) and self.n_tempering_parameters >= 2
            ):
                raise ValueError("UQpy: input n_tempering_parameters should be a integer >= 2.")
            else:
                self.tempering_parameters = [
                    1.0 / np.sqrt(2) ** i for i in range(self.n_tempering_parameters - 1, -1, -1)
                ]
        elif (
    ...
    def run(self, nsamples: PositiveInteger = None, nsamples_per_chain: PositiveInteger = None):
        ...
        while self.mcmc_samplers[0].nsamples_per_chain < final_ns_per_chain:
            ...
            if (
                self.mcmc_samplers[-1].iterations_number > self.mcmc_samplers[-1].burn_length
                and (self.mcmc_samplers[-1].iterations_number - self.mcmc_samplers[-1].burn_length)
                ...
                for t, sampler in enumerate(self.mcmc_samplers):
                    ...
                    if self.save_log_pdf:
                        sampler.log_pdf_values[sampler.nsamples_per_chain, :] = new_log_pdf[
                            t
                        ].copy()
