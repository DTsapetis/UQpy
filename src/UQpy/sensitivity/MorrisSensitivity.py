...
class MorrisSensitivity:
    ...
    def run(self, n_trajectories: PositiveInteger):
        ...
        (trajectories_unit_hypercube, trajectories_physical_space) = self.sample_trajectories(
            n_trajectories=n_trajectories, maximize_dispersion=self.maximize_dispersion
        )
    ...
    def store_data(
        self, elementary_effects, trajectories_physical_space, trajectories_unit_hypercube
    ):
    ...
    def sample_trajectories(
        ...
        for r in range(ntrajectories_all):
            ...
            initial_state = (
                1.0
                / (self.n_levels - 1)
                * randint(low=0, high=int((self.n_levels - 1) * (1 - self.delta) + 1)).rvs(
