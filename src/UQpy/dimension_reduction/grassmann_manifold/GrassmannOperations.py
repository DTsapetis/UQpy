...
class GrassmannOperations:
    ...
    def log_map(
        ...
        """
        reference_point = (
            reference_point.data if isinstance(reference_point, GrassmannPoint) else reference_point
        )
        ...
        for i in range(number_of_points):
            ...
            if reference_point.data.shape[1] != grassmann_points[i].data.shape[1]:
                raise ValueError(
                    "UQpy: Point {0} is on G({1},{2}) - Reference is on G({1},{2})".format(
                        i, grassmann_points[i].data.shape[1], grassmann_points[i].data.shape[0]
                    )
                )
    ...
    @beartype
    def exp_map(
        tangent_points: list[Numpy2DFloatArray], reference_point: Union[np.ndarray, GrassmannPoint]
    ) -> list[GrassmannPoint]:
    ...
    def _stochastic_gradient_descent(data_points, distance_fun, tolerance, maximum_iterations):
        ...
        while itera < maxiter:
            ...
            for i in range(len(indices)):
                ...
                idx = indices[i]
                _gamma = GrassmannOperations.log_map(
                    grassmann_points=[data_points[idx]], reference_point=np.asarray(mean_element)
                )
