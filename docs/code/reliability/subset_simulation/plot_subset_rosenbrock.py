...
mcmc_init1 = ModifiedMetropolisHastings(
    dimension=2,
    log_pdf_target=dist.log_pdf,
    seed=x.tolist(),
    burn_length=1000,
    proposal=[dist_prop1, dist_prop2],
    random_state=8765,
)
...
x_ss_MMH = SubsetSimulation(
    ...
    nsamples_per_subset=10000,
    samples_init=mcmc_init1.samples,
)
...
mcmc_init2 = Stretch(
    dimension=2, log_pdf_target=dist.log_pdf, seed=x.tolist(), burn_length=1000, random_state=8765
)
...
x_ss_Stretch = SubsetSimulation(
    ...
    conditional_probability=0.1,
    nsamples_per_subset=10000,
    samples_init=mcmc_init2.samples,
)
