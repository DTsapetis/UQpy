...
from UQpy.distributions import JointIndependent, MultivariateNormal, Normal, Uniform
...
prior_distribution = JointIndependent(
    marginals=[Uniform(loc=-2, scale=4), Uniform(loc=-2, scale=4)]
)
...
def log_likelihood(x):
    ...
    return np.logaddexp(
        np.log(w1) + multivariate_normal.logpdf(x=x, mean=mu1, cov=sigma1),
        np.log(1.0 - w1) + multivariate_normal.logpdf(x=x, mean=mu2, cov=sigma2),
    )
...
def estimate_evidence_from_prior_samples(size):
    samples = -2.0 + 4 * np.random.uniform(size=size * 2).reshape((size, 2))
    return np.mean(np.exp(log_likelihood(samples)))


def func_integration(x1, x2):
    x = np.array([x1, x2]).reshape((1, 2))
    return np.exp(log_likelihood(x)) * (1.0 / 4) ** 2
...
seed = -2.0 + 4.0 * np.random.rand(100, 2)
mcmc0 = MetropolisHastings(
...
def estimate_evidence_from_posterior_samples(log_posterior_values, posterior_samples):
    log_like = log_likelihood(
        posterior_samples
    )  # log_posterior_values - log_prior(posterior_samples)
    ev = 1.0 / np.mean(1.0 / np.exp(log_like))
    return ev
...
seed = -2.0 + 4.0 * np.random.rand(5, 2)
betas = [1.0 / np.sqrt(2.0) ** i for i in range(20 - 1, -1, -1)]
print(len(betas))
print(betas)

samplers = [
    MetropolisHastings(burn_length=100, jump=3, seed=list(seed), dimension=2)
...
for j, ind in enumerate([0, -6, -1]):
    ax[j].scatter(
        mcmc.mcmc_samplers[ind].samples[:, 0],
        mcmc.mcmc_samplers[ind].samples[:, 1],
        alpha=0.5,
        color="orange",
    )
...
ev = mcmc.evaluate_normalization_constant(compute_potential=compute_potential, log_Z0=0.0)
print("Estimate of evidence by thermodynamic integration = {:.4f}".format(ev))

ev = mcmc.evaluate_normalization_constant(
    compute_potential=compute_potential, nsamples_from_p0=5000
)
print("Estimate of evidence by thermodynamic integration = {:.4f}".format(ev))
...
def indic_sigmoid(y, beta):
    return 1.0 / (1.0 + np.exp(y / (1.0 / beta - 1.0)))


fig, ax = plt.subplots(figsize=(4, 3.5))
ys = np.linspace(-5, 5, 100)
for i, s in enumerate(1.0 / np.array([1.01, 1.25, 2.0, 4.0, 70.0])):
    ax.plot(
        ys,
        indic_sigmoid(y=ys, beta=s),
        label=r"$\beta={:.2f}$".format(s),
        color="blue",
        alpha=1.0 - i / 6,
    )
...
model = RunModel(
    model=PythonModel(
        model_script="local_reliability_funcs.py",
        model_object_name="correlated_gaussian",
        b_eff=beff,
        d=dim,
    )
)
samples = MultivariateNormal(mean=np.zeros((2,)), cov=np.array([[1, 0.7], [0.7, 1]])).rvs(
    nsamples=20000
)
...
mask = model_values <= 0
ax.scatter(
    samples[np.squeeze(mask), 0],
    samples[np.squeeze(mask), 1],
    color="red",
    label="fail",
    alpha=0.5,
    marker="d",
...
betas = (1.0 / np.array([1.01, 1.02, 1.05, 1.1, 1.2, 1.5, 2.0, 3.0, 5.0, 10.0, 25.0, 70.0]))[::-1]
...
samplers = [
    MetropolisHastings(
        ...
        proposal=JointIndependent([Normal(scale=scale)] * 2),
    )
    for scale in scales
]
mcmc = ParallelTemperingMCMC(
    log_pdf_intermediate=log_factor_temp,
...
for j, ind in enumerate([0, 6, -1]):
    ax[j].scatter(
        mcmc.mcmc_samplers[ind].samples[:, 0], mcmc.mcmc_samplers[ind].samples[:, 1], alpha=0.25
    )
    ax[j].set_xlim([-4, 4])
    ax[j].set_ylim([-4, 4])
...
def compute_potential(x, temper_param, log_intermediate_values):
    ...
    indic_beta = np.where(indic_beta > 1.0 - 1e-16, 1.0 - 1e-16, indic_beta)
    indic_beta = np.where(indic_beta < 1e-16, 1e-16, indic_beta)
    tmp_log = np.log((1.0 - indic_beta) / indic_beta)
    return -(1.0 - indic_beta) / (temper_param * (1.0 - temper_param)) * tmp_log
...
plt.plot(
    mcmc.thermodynamic_integration_results["temper_param_list"],
    mcmc.thermodynamic_integration_results["expect_potentials"],
    marker="x",
)
plt.grid(True)
...
for i in range(50):
    ...
    ev = mcmc.evaluate_normalization_constant(
        compute_potential=compute_potential, log_Z0=np.log(0.5)
    )
    # print(np.exp(log_ev))
    list_ev_0.append(ev)
    ev = mcmc.evaluate_normalization_constant(
        compute_potential=compute_potential, nsamples_from_p0=100000
    )
    list_ev_1.append(ev)
