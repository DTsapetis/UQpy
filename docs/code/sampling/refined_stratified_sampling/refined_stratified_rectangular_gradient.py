...
from UQpy.sampling import RefinedStratifiedSampling, TrueStratifiedSampling
...
ID = ax2.scatter3D(
    x.samples[:, 0], x.samples[:, 1], rmodel1.qoi_list, color="Red", label="Input data"
)
...
z = RefinedStratifiedSampling(
    stratified_sampling=x, refinement_algorithm=refinement, random_state=2
)
...
# Plot for scattered data
ID = a4.scatter3D(
    z.samples[:, 0], z.samples[:, 1], rmodel.qoi_list, color="Red", label="Input data"
)
plt.legend(handles=[kr, ID])
plt.show()
