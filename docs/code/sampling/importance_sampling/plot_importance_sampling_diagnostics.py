...
w = ImportanceSampling(
    log_pdf_target=log_Rosenbrock, args_target=(20,), proposal=proposal, nsamples=5000
)
...
effective_sample_size = 1.0 / np.sum(w.weights**2, axis=0)

print(
    "Effective sample size is ne={}, out of a total number of samples={} \n".format(
        effective_sample_size, np.size(w.weights)
    )
)
