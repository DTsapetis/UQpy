...
from UQpy.inference import BIC, MLE, ComputationalModel, InformationModelSelection
...
noise = Normal(loc=0.0, scale=np.sqrt(error_covariance)).rvs(nsamples=50).reshape((50,))
...
for i in range(3):
    ...
    M = ComputationalModel(
        runmodel_object=h_func, n_parameters=i + 1, name=names[i], error_covariance=error_covariance
    )
...
selector = InformationModelSelection(
    parameter_estimators=estimators, criterion=BIC(), n_optimizations=[1] * 3
)
...
print(
    "Values of data fit:",
    [cr - pe for (cr, pe) in zip(selector.criterion_values, selector.penalty_terms)],
)
