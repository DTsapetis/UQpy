...
from UQpy.distributions import ChiSquare, Exponential, Gamma
from UQpy.inference import MLE, DistributionModel, InformationModelSelection
...
m1 = DistributionModel(
    distributions=Exponential(loc=None, scale=None), n_parameters=2, name="exponential"
)
m2 = DistributionModel(
    distributions=ChiSquare(df=None, loc=None, scale=None), n_parameters=3, name="chi-square"
)
...
from UQpy.inference import AIC, BIC, AICc
...
for criterion in criteria:
    selector = InformationModelSelection(
        parameter_estimators=[mle1, mle2, mle3], criterion=criterion, n_optimizations=[5] * 3
    )
    ...
    print(
        ...
        + str(criterion)
        + " criterion: "
        + ", ".join(m.name for m in selector.candidate_models)
    )
