...
from UQpy.inference import MLE, DistributionModel, MinimizeOptimizer
...
d_guess = JointCopula(
    marginals=[Normal(loc=None, scale=None), Normal(loc=None, scale=None)],
...
ml_estimator = MLE(
    ...
    data=data_2,
    optimizer=optimizer,
    initial_parameters=[1.0, 1.0, 1.0, 1.0, 4.0],
)

print(
    "ML estimates of the mean={0:.3f} and std. dev={1:.3f} of 1st marginal (true: 0.0, 1.0)".format(
        ml_estimator.mle[0], ml_estimator.mle[1]
    )
)
print(
    "ML estimates of the mean={0:.3f} and std. dev={1:.3f} of 2nd marginal (true: 0.0, 1.0)".format(
        ml_estimator.mle[2], ml_estimator.mle[3]
    )
)
...
d_guess = JointCopula(
    marginals=[Normal(loc=None, scale=None), Normal(loc=0.0, scale=1.0)], copula=Gumbel(theta=None)
)
candidate_model = DistributionModel(n_parameters=3, distributions=d_guess)

optimizer = MinimizeOptimizer(bounds=[[-5, 5], [0, 10], [1.1, 4]], method="SLSQP")
ml_estimator = MLE(
    inference_model=candidate_model,
    data=data_2,
    optimizer=optimizer,
    initial_parameters=[1.0, 1.0, 4.0],
)
