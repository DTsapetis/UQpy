...
print(
    "ML estimates of the mean={0:.3f} (true=0.) and std. dev={1:.3f} (true=0.1)".format(
...
d = Normal(loc=0.0, scale=None)
candidate_model = DistributionModel(n_parameters=1, distributions=d)

optimizer = MinimizeOptimizer(bounds=[[0.0001, 2.0]])
ml_estimator = MLE(inference_model=candidate_model, data=data_1, n_optimizations=1)
