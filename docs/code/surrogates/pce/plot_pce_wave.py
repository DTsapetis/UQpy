...
def pde_func(s, pce):
    ...
    deriv_0_pce = derivative_basis(
        s, pce, derivative_order=2, leading_variable=0
    ) * transformation_multiplier(pde_data, leading_variable=0, derivation_order=2)
    deriv_1_pce = derivative_basis(
        s, pce, derivative_order=2, leading_variable=1
    ) * transformation_multiplier(pde_data, leading_variable=1, derivation_order=2)
...
def bc_res(nsim, pce):
    ...
    der_order = 0
    deriv_0_pce = np.sum(
        derivative_basis(bc_s, pce, derivative_order=der_order, leading_variable=0)
        * ((2 / 1) ** der_order)
    ...
    bc_init_s = polynomial_chaos.Polynomials.standardize_sample(
        bc_init_x, pce.polynomial_basis.distributions
    )
...
def virt_sampling(nsim=1000):
    domain_samples = LatinHypercubeSampling(
        distributions=joint, criterion=Centered(), nsamples=nsim
    )._samples.reshape(-1, 2)
...
pde_pce = PdePCE(
    ...
    pde_func,
    pde_source=pde_res,
    boundary_conditions_evaluate=bc_res,
